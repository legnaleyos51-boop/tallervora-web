...
async function detectarArticulos() {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        try {
            ...
            foundArticles.push({ url, title, imgSrc, excerpt });
        } catch (e) { break; }
    }

    const sortedArticles = foundArticles.reverse();
    const featuredTitle = document.getElementById('featured-title');
    
    if (featuredTitle && sortedArticles.length > 0) {
        ...
        sortedArticles.slice(1, 6).forEach(art => renderArticleItem(art, lista));
    } else {
        sortedArticles.slice(0, 5).forEach(art => renderArticleItem(art, lista));
    }
}
